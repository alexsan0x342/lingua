...
export default function CodeManagement() {
  ...
  const [newCode, setNewCode] = useState({
    code: '',
    type: 'DISCOUNT' as 'DISCOUNT' | 'COURSE' | 'SERVICE',
    value: 0,
    courseId: '',
    serviceType: '',
    maxUses: 1,
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage discount codes, course access codes, and service codes
          </p>
        </div>
          ...
          <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  Create a new redemption code for discounts, courses, or services.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                ...
                {newCode.type === 'COURSE' && (
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Select value={newCode.courseId} onValueChange={(value) => setNewCode({ ...newCode, courseId: value })}>
                      <SelectTrigger className="col-span-3">
                      ...
                      <SelectContent>
                        {courses.map((course) => (
                          <SelectItem key={course.id} value={course.id}>
                            {course.title} - {course.price / 100} LYD
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                ...
                {newCode.type === 'SERVICE' && (
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input
                      id="service"
                      value={newCode.serviceType}
                      onChange={(e) => setNewCode({ ...newCode, serviceType: e.target.value })}
              ...
              <div className="grid gap-4 py-4">
                ...
                {bulkCode.type === 'COURSE' && (
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Select value={bulkCode.courseId} onValueChange={(value) => setBulkCode({ ...bulkCode, courseId: value })}>
                      <SelectTrigger className="col-span-3">
                        <SelectValue placeholder="Select course" />
                      </SelectTrigger>
                      <SelectContent>
                ...
                {bulkCode.type === 'SERVICE' && (
                  <div className="grid grid-cols-4 items-center gap-4">
                    <Label htmlFor="bulk-service" className="text-right">
                      Service
                    </Label>
                    <Input
                      id="bulk-service"
                      value={bulkCode.serviceType}
                      onChange={(e) => setBulkCode({ ...bulkCode, serviceType: e.target.value })}
                      placeholder="Enter service type"
                    />
                  </div>
                )}
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input
                    id="bulk-expiresAt"
                    type="datetime-local"
                    value={bulkCode.expiresAt}
                    onChange={(e) => setBulkCode({ ...bulkCode, expiresAt: e.target.value })}
                  />
                </div>
              </div>
              <DialogFooter>
            ...
            <TableBody>
              {codes.map((code) => (
                <TableRow key={code.id}>
                  ...
                  <TableCell>
                    <Badge variant={code.type === 'DISCOUNT' ? 'default' : code.type === 'COURSE' ? 'secondary' : 'outline'}>
                      {code.type}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    {code.type === 'DISCOUNT' ? `$${code.value / 100}` : 
                     code.type === 'COURSE' ? code.course?.title : 
                     code.serviceType}
                  </TableCell>
                  <TableCell>
